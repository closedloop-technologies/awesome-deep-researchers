...
| --- | --- | --- |
| OpenAI Deep Research | `python .claude/skills/openai-deep-research/scripts/run_deep_research.py --prompt "$PROMPT"` | prefer mini/deep-research model, cap output, short prompt |
| Perplexity Sonar Deep Research | `python .claude/skills/perplexity-sonar/scripts/ask.py --prompt "$PROMPT"` | use low search/context mode where available |
| Exa Research | `python .claude/skills/exa-research/scripts/exa_tools.py search "$PROMPT" --num-results 5 --highlights` | use search for benchmark smokes; reserve research mode for deeper runs |
| Tavily Search | `python .claude/skills/tavily-search/scripts/tavily_search.py --query "$PROMPT"` | cap results and include answer mode only when needed |
| Jina AI | `python .claude/skills/jina-ai/scripts/jina_tools.py search "$PROMPT"` | cap result count and fetched pages |
| xAI Grok | `python .claude/skills/xai-grok/scripts/grok_research.py --query "$PROMPT"` | use economical model and cap output |
